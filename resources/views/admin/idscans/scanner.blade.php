...
@section('title', 'ID Scanner — Scan New ID')
...
@section('content')
<form id="scanForm" method="POST" action="{{ route('admin.id-scans.store') }}" enctype="multipart/form-data">
    @csrf

    {{-- Step 1: Configuration --}}
    <x-admin.card>
        <x-slot name="title"><i class="bi bi-gear me-2"></i>Step 1 — Document &amp; Renter</x-slot>
        <div class="row g-3">
            <div class="col-md-4">
                ...
                <select id="riderSelect" name="userId" class="form-select">
                    ...
                    @foreach($riders as $rider)
                        <option value="{{ $rider->id }}">
                            {{ $rider->name }} ({{ $rider->email }}){{ $rider->verified ? ' — Verified' : ' — Unverified' }}
                        </option>
                    @endforeach
                </select>
    ...
    <x-admin.card>
        <x-slot name="title"><i class="bi bi-camera-video-fill me-2"></i>Step 2 — Capture Document</x-slot>
        <x-slot name="tools">
    ...
    <x-admin.card>
        <x-slot name="title"><i class="bi bi-person-lines-fill me-2"></i>Step 3 — Extracted Information</x-slot>
    ...
    <div class="d-flex gap-2 mb-5">
        ...
        <a href="{{ route('admin.id-scans.index') }}" class="btn-admin btn-admin--secondary btn-admin--lg">Cancel</a>
    </div>
</form>
@endsection
